...
namespace NuGet.Protocol.Tests
{
    public class DownloadResourceResultTests
    {
        ...
        {
            var exception = Assert.Throws<ArgumentNullException>(
                () => new DownloadResourceResult(stream: null, source: string.Empty));
        ...
        public void Constructor_StreamPackageReaderBase_ThrowsForNullStream()
        {
            ...
            {
                var exception = Assert.Throws<ArgumentNullException>(
                    () => new DownloadResourceResult(stream: null, packageReader: packageReader, source: string.Empty));
        ...
        {
            using (var result = new DownloadResourceResult(Stream.Null, packageReader: null, source: string.Empty))
            {
                Assert.Null(result.PackageReader);
            }
        }
        ...
        public void Dispose_IsIdempotent()
        {
            ...
            using (var result = new DownloadResourceResult(stream, source: string.Empty))
            {
                result.Dispose();
                result.Dispose();
